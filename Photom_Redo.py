# -*- coding: utf-8 -*-
"""
Created on Tue Sep 10 07:57:20 2024

@author: colemaya
"""

'''
For old photometry csvs, run through this code to check if any YB images got
flagged as saturated that shouldn't have
'''
...
out_data = ascii.read(out_name, delimiter=',')
#Set last YB
startpos = 1
YB2 = 6176
...
while (YB1 < YB2) and not done:
    ...
    flagcheck = [out_data[YB1+1]['8flag1'], out_data[YB1+1]['12flag1'], out_data[YB1+1]['24flag1'],
             out_data[YB1+1]['70flag1']]
    
    if flagcheck != ['0']*4 and type(flagcheck[0]) == np.str_:
        ...
        try:
            if flagcheck[3] == '1':
                ...
                coord70 = out_data[YB1+1]['vertices 70']
            ...
            if flagcheck[2] == '1':
                ...
                coord24 = out_data[YB1+1]['vertices 24']
            ...
            if flagcheck[1] == '1':
                ...
                coord12 = out_data[YB1+1]['vertices 12']
            ...
            if flagcheck[0] == '1':
                ...
                coord8 = out_data[YB1+1]['vertices 8']
       
            ##############################################################################
            # Use residual images to perform photometry and write out to table with flags#
            ##############################################################################
            
            #print('YB number: ' + str(YB1 + 1))
            #print('Retry: ' + str(retry != [False]*4))
            ...
            if retry != [False] * 4:
                ...
                if retry[0] == True:
                    findata += [round(flux_tot.um8, 5)]
                elif flaglist[0] == 1:
                    findata += ['']
                else:
                    findata += [out_data[YB1 + 1]['8umphotom']]
                    
                if retry[1] == True:
                    findata += [round(flux_tot.um12, 5)]
                elif flaglist[1] == 1:
                    findata += ['']
                else:
                    findata += [out_data[YB1 + 1]['12umphotom']]
                    
                if retry[2] == True:
                    findata += [round(flux_tot.um24, 5)]
                elif flaglist[2] == 1:
                    findata += ['']
                else:
                    findata += [out_data[YB1 + 1]['24umphotom']]
                    
                if retry[0] == True:
                    findata += [round(flux_tot.um70, 5)]
                elif flaglist[3] == 1:
                    findata += ['']
                else:
                    findata += [out_data[YB1 + 1]['70umphotom']]
