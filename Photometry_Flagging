...
instID= 'coletest - Copy' #Change to be your ID
...
def make_flags(imlist1, imlist2, imlist3, imgwlist, YB):
    ...
    flagnames= ["Multiple sources within masked area", "No Obvious Source at 70 um",
                "No Obvious Source at 24 um", "No Obvious Source at 12 um", "No Obvious Source at 8 um",
                 ...
                 "Poor Confidence in 12 um Photometry", "Poor Confidence in 8 um Photometry"]
    ...
    for i in range(len(imlist1)):
        ...
        plt.subplot(4, 4, 1 + 4*i, projection= imgwlist[i])
        if i == 0:
    ...
    for i in range(len(imlist3)):
        img3= imlist3[i]
        plt.subplot(4, 4, 3 + 4*i, projection= imgwlist[i])
        if i == 0:
    ...
    checkaxes= plt.axes([.7, .575, .225, .305])
...
while (YB1 < YB2) and not done:
    ...
    for i in range(len(wavelengths)):
        ...
        if coordinates != ' ':
            interp= do_interp(workmasks[i], ast.literal_eval(coordinates))
            interps.append(interp.interp)
            resids.append(interp.resid)
    ...
    flag= make_flags(workmasks, resids, interps, wcslist, YB)
