#################################################
#
# You will need access to a valid API to load data from the google drive with this
# Works off of computer files as well
#
#################################################
...
instID = 'EthanBass' #Change to be your ID
...
class choose_image():
    def __init__(self):
        ...
        self.um70data = None 
        self.um70w = None  
    #grabs the mosaics from Google drive, assuming the user has the right json file. 
    def update_mosaic_api(self, l, b):
        ...
        #if we already have the right mosaics downloaded, do nothing. 
        if(self.path8 == path8 and self.path12 == path12 and self.path24==path24 and self.path70 ==path70):
            return 0

        #Searches Google Drive for the right file, and then we download it to the computer
        #Sadly there seems to be no way to just 'stream' it from Drive
        #Additionally having it on the computer saves memory and prevents crashes.
        #Only ever has 4 mosaics downloaded and the files are removed from the computer at the end of the program 
        ...
        #os.remove(temp_path) This line makes it run worse, remove the files at the end.
    
    #Update mosaics from the files on the user computers. 
    def update_mosaic(self, l, b):
        ...
        if not (os.path.exists(path8) and os.path.exists(path12) and os.path.exists(path24) and os.path.exists(path70)):
            return 1
        
        #If we already have the right mosaics, no need to open and reassign the same values
        if(self.path8 == path8 and self.path12 == path12 and self.path24==path24 and self.path70 ==path70):
            return 0
        self.path8 = path8
        self.path12 = path12
...
while (YB1 < YB2) and not done:
    ...
    YB_lat = data[YB1]['b']
    YB_rad = data[YB1]['r']
    if (image.update_mosaic(YB_long, YB_lat) == 1):
        image.update_mosaic_api(YB_long, YB_lat)
